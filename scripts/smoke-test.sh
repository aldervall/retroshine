#!/bin/bash
# smoke-test.sh - Automated smoke tests for retro-gaming-sunshine container
# Run AFTER container is started: bash smoke-test.sh

FAIL=0

echo "=== Retro Gaming Sunshine Smoke Tests ==="
echo ""

# 1. Container running
docker ps --format '{{.Names}}' | grep -q retro-sunshine && \
    echo "✅ Container running" || { echo "❌ Container not running"; FAIL=1; }

# 2. Sunshine process alive
docker exec retro-sunshine pgrep sunshine > /dev/null && \
    echo "✅ Sunshine process running" || { echo "❌ Sunshine not running"; FAIL=1; }

# 3. Xvfb running
docker exec retro-sunshine pgrep Xvfb > /dev/null && \
...
docker exec retro-sunshine grep -Eq "encoder = (vaapi|nvenc)" /config/sunshine.conf && \
    echo "✅ $(docker exec retro-sunshine grep -oP 'encoder = \K\w+' /config/sunshine.conf) configured" || { echo "❌ Encoder not configured"; FAIL=1; }

# 6. ES-DE installed
docker exec retro-sunshine which es-de > /dev/null && \
    echo "✅ ES-DE installed" || { echo "❌ ES-DE missing"; FAIL=1; }

# 7. RetroArch installed
docker exec retro-sunshine which retroarch > /dev/null && \
...
CORE_COUNT=$(docker exec retro-sunshine sh -c 'ls -1 /usr/lib/x86_64-linux-gnu/libretro/*.so 2>/dev/null' | wc -l)
...
# 9. uinput available
docker exec retro-sunshine test -c /dev/uinput && \
    echo "✅ uinput available" || { echo "❌ uinput missing"; FAIL=1; }

# 10. NVIDIA GPU detected
docker exec retro-sunshine nvidia-smi -L 2>/dev/null | grep -q "Quadro P1000" && \
